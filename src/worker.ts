...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // ==========================================
    // ROTAS DE TAREFAS & MINHA ROTINA
    // ==========================================

    // GET /api/tasks/my-day: Retorna tarefas agrupadas do usuário logado
    if (path === "/api/tasks/my-day") {
      if (request.method !== "GET") {
        return errorResponse("Método não permitido.", 405);
      }
      ...
      try {
        ...
        // Check for stale clients (> 15 days without contact)
        ...
        for (const c of clientsResult.results || []) {
        ...
        console.error("[Worker Tasks API] Erro ao buscar my-day:", err);
        return errorResponse(err.message || "Erro ao consultar tarefas.", 500);
      }
    }

    // PATCH /api/tasks/:id/complete e POST /api/tasks/:id/complete
    if (path.startsWith("/api/tasks/") && path.endsWith("/complete")) {
      ...
      if (env.DB) {
        try {
          ...
          console.error("[Worker Task Complete] Erro:", err);
          return errorResponse(err.message || "Erro ao concluir tarefa.", 500);
        }
      }

      return jsonResponse({ success: true, message: "Tarefa concluída com sucesso.", taskId });
    }

    // PATCH /api/tasks/:id/reschedule e POST /api/tasks/:id/reschedule
    if (path.startsWith("/api/tasks/") && path.endsWith("/reschedule")) {
      if (request.method !== "PATCH" && request.method !== "POST") {
        return errorResponse("Método não permitido.", 405);
      }
      ...
      if (!dueDate) {
        return errorResponse("dueDate é obrigatório.", 400);
      }
      ...
      if (env.DB) {
        try {
          ...
          return jsonResponse({ success: true, message: "Tarefa reagendada com sucesso." });
        } catch (err: any) {
          return errorResponse(err.message || "Erro ao reagendar tarefa.", 500);
        }
      }
